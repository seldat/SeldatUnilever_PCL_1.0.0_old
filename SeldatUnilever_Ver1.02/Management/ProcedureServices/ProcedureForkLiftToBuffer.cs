            ...
            while (ProRun)
            {
                switch (StateForkLift)
                {
                    ...
                    case ForkLift.FORBUF_ROBOT_GOTO_BACK_FRONTLINE_READY:
                        // kiểm tra có robot nào đang trong vung Gate
                    ...
                    case ForkLift.FORBUF_ROBOT_WAITTING_GOTO_GATE_READY:
                        ...
                        //  if (Traffic.RobotIsInArea("OPEN_GATE", rb.properties.pose.Position)) // khu vuc mo cong
                    ...
                    case ForkLift.FORBUF_ROBOT_WAITTING_GOTO_GATE_READY_OPEN_DOOR:
                        ...
                        break;
                    case ForkLift.FORBUF_ROBOT_WAITTING_ALLOW_GOTO_GATE_FROM_OUTER:
                        // kiem tra vung đăng ký tai khu vuc xac định
                        if (TrafficRountineConstants.DetetectInsideStationCheckGate(registryRobotJourney))
                        {
                            break;
                        }
                        else
                        {
                            // dò ra điểm đích đến và xóa đăng ký vùng
                            StateForkLift= ForkLift.FORBUF_ROBOT_WAITTING_GOTO_INSIDE_GATE_ZONE;
                        }
                        break;
                    case ForkLift.FORBUF_ROBOT_WAITTING_GOTO_INSIDE_GATE_ZONE:
                        // kiem tra vung đăng ký tai khu vuc xac định
                        //TrafficRountineConstants.DetectRelease(registryRobotJourney);
                       if (Traffic.RobotIsInArea("GATE", rb.properties.pose.Position))
                        ...
                        break;
                    case ForkLift.FORBUF_ROBOT_WAITTING_GOTO_GATE_OPEN_DOOR:
                        // kiem tra vung đăng ký tai khu vuc xac định
                        TrafficRountineConstants.DetectRelease(registryRobotJourney);
                        if (resCmd == ResponseCommand.RESPONSE_LASER_CAME_POINT)
                        {
                    ...
                    case ForkLift.FORBUF_ROBOT_WAITTING_ZONE_BUFFER_READY:
                        ...
                        // kiểm tra xem goal có đếm Merz
                        // kiểm tra goal có nằm trong vung c1
